...
const formatarTitulo = function (titulo){
    ...
    for(let indice in array){
        ...
        if(array[indice].length > 3 || Number.parseInt(indice) === 0){
           const letras = array[indice].split('');           
           letras[0] = letras[0].toUpperCase();

           array[indice] = letras.join('');
        }

        resposta.push(array[indice]);
    }

    return resposta.join(' ');
}

//const formatarTitulo = filmes.map(f => 
//                        (f = {...f,titulo:f.duracao}));

filmes.forEach(f => console.log(formatarTitulo(f.titulo)));